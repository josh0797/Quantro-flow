...
/**
 * PlanAndUsage — consolidated account center (plan + API usage + billing + account).
 *
 * Four stacked cards mirroring the user's spec:
 *   1. Plan Actual           — name, status badge, renewal, CTAs
 *   2. Uso de API            — total vs monthly limit, progress, module breakdown
 *   3. Facturación           — payment method, next billing, manage CTA
 *   4. Cuenta                — user/workspace, switch account, sign out
 *
 * Pulls live data from GET /api/usage. The Cuenta block mirrors AuthContext so
 * the user's identity stays in sync with the sidebar menu.
 */
export default function PlanAndUsage() {
  ...
  useEffect(() => {
    ...
    const fetchUsage = async () => {
      try {
        ...
        const res = await authFetch(`${backendUrl}/api/usage`);
        if (!res.ok) throw new Error('usage failed');
        const json = await res.json();
        if (!cancelled) setData(json);
      } catch (_) {
        // keep empty state; page still renders account/billing defaults
      } finally {
  ...
  const statusBadge = () => {
    const status = data?.plan?.status || 'trial';
    const map = {
    ...
    return <Badge data-testid="plan-status-badge" className={`text-[10px] font-medium tracking-wide uppercase ${m.cls} border`}>{m.label}</Badge>;
  };
            ...
            <div>
              ...
              <div className="flex items-baseline gap-3 mt-1">
                <span data-testid="plan-name" className="text-xl font-semibold text-foreground">
                  {loading ? <Skeleton className="h-6 w-24 inline-block align-middle" /> : data?.plan?.name || 'Starter'}
                </span>
                {!loading && statusBadge()}
              </div>
            </div>
        ...
        <CardContent className="relative flex flex-col md:flex-row md:items-center gap-4 md:gap-6 justify-between">
          <div className="text-xs text-muted-foreground">
            ...
            <span data-testid="plan-renewal-date" className="text-foreground font-medium">
              {loading ? '—' : formatDate(data?.plan?.renewal_date)}
            </span>
          </div>
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-end justify-between gap-4">
                <div>
                  <div className="text-2xl font-semibold text-foreground tabular-nums">
                    <span data-testid="usage-total">{data?.usage?.total_calls?.toLocaleString() || 0}</span>
                    <span className="text-sm text-muted-foreground font-normal">
                      {' '}/ <span data-testid="usage-limit">{data?.usage?.monthly_limit?.toLocaleString() || 0}</span> {t('plan_usage.calls')}
                    </span>
                  </div>
              ...
              <Progress
                ...
                value={data?.usage?.usage_percent || 0}
              ...
              <div className="pt-3 border-t border-[hsl(var(--border))]">
                ...
                <div className="space-y-3" data-testid="usage-breakdown">
                  {(data?.usage?.breakdown || []).map((row) => (
                    <div key={row.module} className="flex items-center gap-3" data-testid={`usage-row-${row.module}`}>
          ...
          <div className="space-y-1.5 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span data-testid="billing-method" className="text-foreground font-medium">
                {loading ? '—' : (data?.billing?.payment_method || t('plan_usage.no_payment_method'))}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span data-testid="billing-next-date" className="text-foreground font-medium">
                {loading ? '—' : formatDate(data?.billing?.next_billing_date)}
              </span>
            </div>
          </div>
          <Button
